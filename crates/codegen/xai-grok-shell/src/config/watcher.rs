...
const VENDOR_CONFIG_ROOT_NAMES: &[&str] = &[".grok", ".agents"];
...
impl SkillsFileWatcher {
    ...
    /// [`collect_skill_config_dirs`](xai_grok_agent::prompt::skills::collect_skill_config_dirs).
    ///
...
mod tests {
    ...
    fn vendor_skill_refresh_dirs_paths_and_modes() {
        let root = Path::new("/tmp/project/.grok");
        assert_eq!(
    ...
    fn plan_skills_watch_targets_scopes_vendors_and_seeds_refresh() {
        ...
        let dirs = vec![project_agents.clone(), project_grok.clone(), custom.clone()];
        let plan = plan_skills_watch_targets(&dirs, &grok_home, Some(project));

        assert_eq!(
            plan.vendor_roots,
            vec![project_agents.clone(), project_grok.clone()]
        );
        assert_eq!(plan.recursive_roots, vec![custom]);
        // Both vendor roots are present, so nothing is seeded and no
        // parent watch is needed.
        assert_eq!(plan.project_parent_watch.as_deref(), None);

        let mut expected_refresh: Vec<(PathBuf, RecursiveMode)> =
            vendor_skill_refresh_dirs(&project_agents)
                .into_iter()
    ...
    fn plan_skills_watch_targets_partial_vendors_seed_only_missing() {
        ...
        let plan = plan_skills_watch_targets(
            std::slice::from_ref(&project_agents),
            &grok_home,
            Some(project),
        );

        assert_eq!(plan.vendor_roots, vec![project_agents.clone()]);
        assert_eq!(plan.project_parent_watch.as_deref(), Some(project));

        let mut expected = vendor_skill_refresh_dirs(&project_agents).to_vec();
        let missing_grok = project.join(".grok");
        expected.push((missing_grok.clone(), RecursiveMode::NonRecursive));
        expected.extend(vendor_skill_refresh_dirs(&missing_grok));
        assert_eq!(plan.refresh_dirs, expected);
        assert!(
            !plan
                .refresh_dirs
                .contains(&(project_agents.clone(), RecursiveMode::NonRecursive))
        );
    }
    ...
    #[test]
    #[cfg(target_os = "linux")]
    fn watch_skill_subdirs_scopes_vendor_root_not_worktrees() {
        let tmp = TempDir::new().unwrap();
        let project_vendor = tmp.path().join(".agents");

        let alpha = project_vendor.join("skills").join("alpha");
        fs::create_dir_all(&alpha).unwrap();
        fs::write(alpha.join("SKILL.md"), "# alpha").unwrap();

        let wt_skill = project_vendor
            .join("worktrees")
            .join("wt1")
            .join("bazel-out")
            .join("deep")
        ...
        let watched = watch_skill_subdirs(&mut debouncer, &project_vendor);
        assert!(watched.contains(&project_vendor.join("skills")));
        wait_ms(150);
        while rx.try_recv().is_ok() {}
        ...
        assert!(
            rx.try_recv().is_err(),
            "changes under project vendor worktrees must not fire"
        );
        ...
        assert!(
            rx.try_recv().is_ok(),
            "changes under project vendor skills must fire"
        );
    }
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        ...
        assert_eq!(
            discovery_change_for_path(Path::new("/tmp/project/.agents")),
            Some(DiscoveryChange::Skills)
        );
        assert_eq!(
