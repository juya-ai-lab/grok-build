...
| `docs: default npm package in artifact checklist` | 考虑 npm wrapper 通常分发或调用同一平台产物，将 npm package 设为默认勾选；模板同时要求在明确不涉及时取消，避免把默认推断当成事实 | 宗旨 3：按实际影响链提高默认覆盖率，同时保留准确修正入口 |
