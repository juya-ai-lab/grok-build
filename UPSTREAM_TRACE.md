...
- 落后上游 0 提交 / 本 fork 领先 135 提交; 树差异 189 个文件
...
| 提交 | 日期 | 内容 |
|---|---|---|
| 4422aafa | 2026-08-06 | docs: record CI release download resilience decision |
| b4f96548 | 2026-08-05 | chore: refresh upstream trace [skip ci] |
| 2ed8c879 | 2026-08-06 | ci: make release ripgrep download resilient |
| 41397e90 | 2026-08-05 | chore: refresh upstream trace [skip ci] |
...
| fd6d6db7 | 2026-08-04 | docs: record upstream rpc batch |
| 19548365 | 2026-08-04 | fix: recognize legacy unknown workspace methods |
| be32c9a4 | 2026-08-04 | chore: refresh upstream trace [skip ci] |
| 808628ae | 2026-08-04 | docs: record upstream auth batch |
| 8eb35738 | 2026-08-04 | fix: repair interactive external auth flow |
| e45737c1 | 2026-08-04 | chore: refresh upstream trace [skip ci] |
...
| 0d72ccd7 | 2026-08-04 | fix: reuse watcher coverage for subagents |
| 4f44478f | 2026-08-04 | docs: record watcher and git head batches |
...
| 上游 0.2.119 选择性同步 | `upstream/main=e5478eff`，`SOURCE_REV=27d2088ae3b3f25e9ddab462caa18a07005ada9a`；已完成全量 diff/调用链/依赖/构建发布路径审计，并以 `82537ac2` 做真实 `--no-ff` merge，选择性合入 project-picker/model/TUI、版本/lock/release 文档及安全认证测试 | 后续上游更新继续按全量审计和分批验证处理；SkillTrigger/skill_md_read 外部遥测仍明确排除，async-openai 仅复用已有 `juya-ai-lab@e03c366c` 例外 | 已完成选择性合并；待推送 main、创建 `v0.2.119` 并观察 Release CI |
...
| `c7af57ca` | 澄清交接快照的选择性同步锚点与随后文档/机械刷新提交，避免把脚本排除的 trace 刷新提交误认成业务状态 | 宗旨 3：交接记录与 Git 事实一致；不改变运行时行为 |
...
| `0d72ccd7` | 仅合入上游 `handle_request` 中的 watcher 覆盖判断：子代理工作目录是否已由父 watcher 覆盖统一使用 `xai_fsnotify::watch_root_covers`；同一上游 diff 中的凭据统一日志、配置/目录元数据和上传/trace 改动明确排除 | 上游批次 B2-SW；宗旨 1（外部敏感数据不进入日志/上传边界）+ 宗旨 2（保留子代理本地 hunk tracking）；`xai-grok-shell` `subagent` 定向测试 315 passed、0 failed，未更新版本或 SOURCE_REV |
...
| `19548365` | 增加 workspace RPC unknown-method 新旧 wire shape 的统一识别：接受 `code=unknown_method`，也接受旧 `hub_error` 消息前缀；只扩展错误分类辅助方法和单元测试 | 上游 0.2.119 RPC 兼容批次；不读取路径/配置、不改变 relay payload、不新增上传或遥测 |
| `fd6d6db7` | 更新 CHANGELOG 与临时审查摘要，记录 RPC unknown-method 兼容批次和验证范围；不改变运行时行为 | 宗旨 3：使用者摘要与维护者决策记录同步 |
| `d3a1ca17` | 更新上游 0.2.119 临时事项：记录 72 文件全量审计已完成第一轮、B1/B2/隐私/认证/RPC 已选择性合入，并明确 picker/model/TUI、SkillTrigger telemetry、版本/lock/release 仍暂缓 | 宗旨 1（隐私边界）+ 宗旨 3（同步状态可追溯）；不改变运行时行为 |
...
| `808628ae` | 更新 CHANGELOG 与临时审查同步后的认证批次摘要，明确交互式 provider、headless refresh 和锁修复的范围；不改变运行时行为 | 宗旨 3：使用者摘要与维护者决策记录保持同步 |
