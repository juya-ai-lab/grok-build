...
- 隐私审计范围以整个上游 diff 和新增调用链为准，不能只复查历史上裁剪过的文件或已知功能；对新增/修改的代码、配置、依赖、构建与发布路径一并检查工作区/路径、提示词/工具、凭证等数据是否进入持久化、遥测、上传、外部通信或第三方兼容面。审计结论和未决项记录在 `UPSTREAM_TRACE.md`；无法证明不触碰边界的路径先暂停合入并确认。
...
- GitHub issue 记录：按适用的 `.github/ISSUE_TEMPLATE/` 模板填写，保留问题范围、上游与本 fork 的版本/commit 来源及验证状态；scope/status 标签按 `CONTRIBUTING.md` 的证据规则维护，安全问题遵循 `SECURITY.md`。
